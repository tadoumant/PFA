<?php
    ...
    if(isset($_REQUEST['Delet'])){
        $ID_Etu=$_REQUEST['Delet'];
        $removeEtu1=$database->prepare("DELETE FROM NoteEtu WHERE ID_Etu=:ID_Etu");
        $removeEtu1->bindParam('ID_Etu',$ID_Etu);
        if($removeEtu1->execute()){
            $removeEtu3=$database->prepare("DELETE FROM Paiments WHERE ID_Etu=:ID_Etu");
            $removeEtu3->bindParam('ID_Etu',$ID_Etu);
            $removeEtu3->execute();
            $removeEtu4=$database->prepare("DELETE FROM DammendAtte WHERE ID_Etu=:ID_Etu");
            $removeEtu4->bindParam('ID_Etu',$ID_Etu);
            $removeEtu4->execute();
            $removeEtu2=$database->prepare("DELETE FROM Etudiants WHERE ID_Etu=:ID_Etu");
            $removeEtu2->bindParam('ID_Etu',$ID_Etu);
            $removeEtu2->execute();
            header("refresh:1;url=Info_Etu.php");
        }
    }elseif(isset($_REQUEST['Update1'])){
        session_start();
        $_SESSION['ID_EtUp']=$_REQUEST['Update1'];
        header("refresh:1;url=UpdateEtu.php");
    }
